...
class NN:
    ...
    def __init__(self, input_dim=None, output_dim=None, hidden_layers=None,AvgWeights=None):
        ...
        self.network = self._build_network(AvgWeights)
        # return self
    ...
    def _build_network(self,AvgWeights=None):
        ...
        if type(AvgWeights) == list:
            network = []
            if len(self.hidden_layers) == 0:
                network.append(_layer(self.input_dim,self.output_dim,AvgWeights[0]))

            else:
                network.append(_layer(self.input_dim, self.hidden_layers[0],AvgWeights[0]))
                for i in range(1, len(self.hidden_layers)):
                    network.append(_layer(self.hidden_layers[i-1], self.hidden_layers[i],AvgWeights[i]))
                network.append(_layer(self.hidden_layers[-1], self.output_dim,AvgWeights[-1]))
